...
function Room() {
  ...
  const wall = useMemo(() => mat("#1d1b1b", 0.92), []);
  return <group>
...
function WindowFrame() {
  ...
  return <group position={[-3.35,2.7,-4.35]}>
    ...
    <pointLight position={[0,0.4,0.6]} color="#7597ff" intensity={2.5} distance={7} decay={2}/>
  </group>;
}
...
function Desk() {
  ...
  return <group position={[2.0,0,-2.35]}>
    ...
    <Chair/>
    <rectAreaLight position={[0,2.65,-0.4]} width={4.4} height={1.8} intensity={5.5} color="#ffc17d"/>
    <pointLight position={[0,1.45,0.15]} color="#ff9c55" intensity={9} distance={5.4} decay={2}/>
  </group>;
}
...
function Dumbbell({position,scale=1}) {return <group position={position} scale={scale}><mesh rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.045,0.045,0.5,16]}/><meshStandardMaterial color="#2a2d31" metalness={0.65}/></mesh>{[-0.3,0.3].map(x=><mesh key={x} position={[x,0,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.15,0.15,0.15,16]}/><meshStandardMaterial color="#17191d" metalness={0.45}/></mesh>)}</group>}
...
function Gym() {
  ...
  return <group position={[-3.55,0,-1.15]} scale={0.78}>
    ...
    <Box args={[4.65,0.03,0.04]} position={[0,0.13,1.73]} color="#ff9b50" castShadow={false}/>
    <group position={[-1.35,0,-0.2]}>{[-0.72,0.72].map(x=><Box key={x} args={[0.13,2.4,0.18]} position={[x,1.22,0]} color="#171a1e" metalness={0.55}/>) }<mesh position={[0,1.95,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.05,0.05,2.35,18]}/><meshStandardMaterial color="#171a1e" metalness={0.6}/></mesh>{[-1.23,1.23].map(x=><mesh key={x} position={[x,1.95,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.28,0.28,0.16,22]}/><meshStandardMaterial color="#171a1e" metalness={0.55}/></mesh>)}</group>
    <RoundedBox args={[1.85,0.25,0.62]} radius={0.08} smoothness={4} position={[-0.5,0.5,0.72]}><meshStandardMaterial color="#171a1e" roughness={0.5} metalness={0.4}/></RoundedBox>
    <group position={[1.3,0,-0.25]}><Box args={[1.8,0.1,0.5]} position={[0,0.42,0]} color="#171a1e" metalness={0.52}/><Box args={[1.8,0.1,0.5]} position={[0,0.86,0]} color="#171a1e" metalness={0.52}/>{[-0.62,-0.2,0.22,0.64].map((x,i)=><group key={x}><Dumbbell position={[x,0.58,0]} scale={0.66+i*0.04}/><Dumbbell position={[x,1.0,0]} scale={0.62+i*0.04}/></group>)}</group>
  </group>;
}
...
function Lighting(){return <><ambientLight intensity={0.34} color="#d5ddeb"/><hemisphereLight intensity={0.52} color="#7899cb" groundColor="#24150e"/><directionalLight castShadow position={[-3,6,5]} intensity={1.55} color="#a9c3ef" shadow-mapSize-width={2048} shadow-mapSize-height={2048}/><pointLight position={[-3.5,2.8,-3.7]} color="#7195ff" intensity={3.8} distance={7} decay={2}/><pointLight position={[2.7,2.2,-3.4]} color="#ffab62" intensity={7.2} distance={6} decay={2}/></>}

export default function Scene01(){return <group><Lighting/><Room/><WindowFrame/><Shelf/><Poster position={[5.2,3.0,-4.4]} lines={["DISCIPLINA","ENFOQUE","CONSTANCIA","ÉXITO"]}/><Poster position={[6.05,3.0,-4.4]} lines={["</>","CODE.","BUILD.","INSPIRE."]}/><Desk/><Gym/><Plant position={[-5.85,0,-3.55]} scale={1.05}/><Plant position={[6.0,0,-3.55]} scale={0.9}/><ContactShadows position={[0,0.08,0]} opacity={0.38} scale={13} blur={2.7} far={7}/></group>}
