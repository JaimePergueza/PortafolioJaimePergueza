...
function WeightPlate({ x, thickness, radius }) {
  return <mesh position={[x,1.68,-0.52]} rotation={[0,0,Math.PI/2]} castShadow receiveShadow>
    <cylinderGeometry args={[radius,radius,thickness,32]}/>
    <meshStandardMaterial color="#171a1e" roughness={0.5} metalness={0.28}/>
  </mesh>;
}
...
function BenchPress() {
  ...
  return <group position={[-3.45,0,-1.18]}>
    <mesh position={[0,0.055,0]} receiveShadow>
      ...
      <meshStandardMaterial color="#101215" roughness={0.96}/>
    </mesh>
    <Box args={[4.38,0.025,0.035]} position={[0,0.12,1.62]} color="#3b74b8" castShadow={false}/>
    <Box args={[4.38,0.025,0.035]} position={[0,0.12,-1.62]} color="#3b74b8" castShadow={false}/>

    <group position={[0,0,-0.52]}>
      {[-1.05,1.05].map(x=><group key={x}>
        <Box args={[0.17,1.62,0.2]} position={[x,0.86,0]} color={steel} metalness={0.58}/>
        <Box args={[0.82,0.15,0.62]} position={[x,0.09,0.08]} color={steel} metalness={0.58}/>
        <Box args={[0.12,0.65,0.15]} position={[x,0.43,0.42]} rotation={[-0.62,0,0]} color={steel} metalness={0.58}/>
        <Box args={[0.29,0.13,0.34]} position={[x,1.52,0.05]} color="#303640" metalness={0.65}/>
        <Box args={[0.1,0.24,0.22]} position={[x,1.62,0.12]} color="#303640" metalness={0.65}/>
      </group>)}
      <mesh position={[0,1.68,0]} rotation={[0,0,Math.PI/2]} castShadow receiveShadow>
        <cylinderGeometry args={[0.065,0.065,3.55,24]}/>
        <meshStandardMaterial color="#aeb7c2" roughness={0.25} metalness={0.88}/>
      </mesh>
      <mesh position={[-1.86,1.68,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.09,0.09,0.2,24]}/><meshStandardMaterial color="#8e98a5" metalness={0.9}/></mesh>
      <mesh position={[1.86,1.68,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.09,0.09,0.2,24]}/><meshStandardMaterial color="#8e98a5" metalness={0.9}/></mesh>
      <WeightPlate x={-1.48} thickness={0.18} radius={0.36}/><WeightPlate x={-1.28} thickness={0.16} radius={0.34}/><WeightPlate x={-1.10} thickness={0.14} radius={0.31}/>
      <WeightPlate x={1.10} thickness={0.14} radius={0.31}/><WeightPlate x={1.28} thickness={0.16} radius={0.34}/><WeightPlate x={1.48} thickness={0.18} radius={0.36}/>
      <mesh position={[-1.68,1.68,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.105,0.105,0.08,24]}/><meshStandardMaterial color="#555e69" metalness={0.82}/></mesh>
      <mesh position={[1.68,1.68,0]} rotation={[0,0,Math.PI/2]} castShadow><cylinderGeometry args={[0.105,0.105,0.08,24]}/><meshStandardMaterial color="#555e69" metalness={0.82}/></mesh>
    </group>

    <group position={[0,0,0.55]}>
      <RoundedBox args={[2.15,0.22,0.62]} radius={0.08} smoothness={4} position={[0,0.52,0]} castShadow receiveShadow>
        <meshStandardMaterial color="#111419" roughness={0.62} metalness={0.12}/>
      </RoundedBox>
      <Box args={[0.14,0.82,0.14]} position={[-0.76,0.24,0]} color={steel} metalness={0.58}/>
      <Box args={[0.14,0.82,0.14]} position={[0.76,0.24,0]} color={steel} metalness={0.58}/>
      <Box args={[0.72,0.12,0.62]} position={[-0.76,0.07,0]} color={steel} metalness={0.58}/>
      <Box args={[0.72,0.12,0.62]} position={[0.76,0.07,0]} color={steel} metalness={0.58}/>
    </group>

    <rectAreaLight position={[0,2.75,0.15]} rotation={[-Math.PI/2,0,0]} width={3.5} height={1.6} intensity={4.4} color="#9bc4ff"/>
    <pointLight position={[-1.7,1.6,0.5]} color="#6ea6ff" intensity={2.4} distance={4.5} decay={2}/>
  </group>;
}
...
export default function Scene01(){return <group><Lighting/><Room/><WindowFrame/><Shelf/><Poster position={[5.2,3.0,-4.4]} lines={["DISCIPLINA","ENFOQUE","CONSTANCIA","ÉXITO"]}/><Poster position={[6.05,3.0,-4.4]} lines={["</>","CODE.","BUILD.","INSPIRE."]}/><Desk/><BenchPress/><Plant position={[6.0,0,-3.55]} scale={0.9}/><ContactShadows position={[0,0.08,0]} opacity={0.52} scale={13} blur={2.2} far={8}/></group>}
